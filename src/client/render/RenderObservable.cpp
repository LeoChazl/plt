 ...
 */
void const RenderObservable::notifyRenderObservers(engine::EngineRenderEvent& engineRenderEvent, state::State& state, state::Position& position, shared_ptr<state::MobileEntity>& selectedMobileEntity){
	for(auto observer : observers){
		observer->engineRenderChanged(engineRenderEvent, state, position, selectedMobileEntity);
	}
}
